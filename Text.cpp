...
std::vector<std::string> Text::loadedFonts;
std::map<GLchar, Text::Character> Text::Characters;
unsigned int Text::VAO, Text::VBO;

void Text::LoadFont(std::string fontFile)
{
    // FreeType
      // --------
    FT_Library ft;
...
Text::Text(Shader shader, std::string text, glm::vec3 translation, glm::quat rotation, glm::vec3 scale, Player& player) :
    text(text),
